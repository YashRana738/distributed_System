class Node:
    def __init__(self, node_id, total_nodes):
        self.id = node_id
        self.total_nodes = total_nodes

        self.clock = 0
        self.requesting = False
        self.in_cs = False
        self.timestamp = None
        self.replies_received = 0
        self.deferred = []

        self.nodes = []

    def set_nodes(self, nodes):
        self.nodes = nodes

    def increment_clock(self):
        self.clock += 1

    def update_clock(self, received_ts):
        self.clock = max(self.clock, received_ts) + 1

    def request_cs(self):
        if self.in_cs:
            print(f"[Node {self.id}] already in CS!")
            return

        self.increment_clock()
        self.requesting = True
        self.timestamp = self.clock
        self.replies_received = 0

        print(f"\n[Node {self.id}] REQUEST CS at time {self.timestamp}")

        for node in self.nodes:
            if node.id != self.id:
                node.receive_request(self.id, self.timestamp)

    def receive_request(self, sender_id, sender_ts):
        self.update_clock(sender_ts)

        sender = self.nodes[sender_id]

        if (not self.requesting and not self.in_cs) or \
           (self.requesting and (sender_ts < self.timestamp or
            (sender_ts == self.timestamp and sender_id < self.id))):

            print(f"[Node {self.id}] → REPLY to Node {sender_id}")
            sender.receive_reply(self.id)
        else:
            print(f"[Node {self.id}] → DEFER Node {sender_id}")
            self.deferred.append(sender_id)

    def receive_reply(self, sender_id):
        self.replies_received += 1
        print(f"[Node {self.id}] received REPLY from {sender_id}")

        if self.replies_received == self.total_nodes - 1:
            self.enter_cs()

    def enter_cs(self):
        self.in_cs = True
        self.requesting = False
        print(f"\n>>> Node {self.id} ENTERS CS <<<\n")

    def exit_cs(self):
        print(f"\n[Node {self.id}] EXIT CS")

        self.in_cs = False
        self.timestamp = None

        if self.deferred:
            print(f"[Node {self.id}] Processing deferred queue: {self.deferred}")

        for node_id in self.deferred:
            print(f"[Node {self.id}] → Deferred REPLY to {node_id}")
            self.nodes[node_id].receive_reply(self.id)

        self.deferred.clear()

def simulate():
    n = int(input("Enter number of nodes: "))
    nodes = [Node(i, n) for i in range(n)]

    for node in nodes:
        node.set_nodes(nodes)

    while True:
        print("\nOptions:")
        print("1. Request CS")
        print("2. Exit CS (Auto)")
        print("3. Show Status")
        print("4. Exit Program")

        choice = input("Enter choice: ")

        if choice == "1":
            node_id = int(input(f"Enter node ID (0 to {n-1}): "))
            if 0 <= node_id < n:
                nodes[node_id].request_cs()
            else:
                print("Invalid node ID!")

        elif choice == "2":

            exited = False
            for node in nodes:
                if node.in_cs:
                    print(f"\n[System] Node {node.id} is in CS → exiting...")
                    node.exit_cs()
                    exited = True
                    break

            if not exited:
                print("[System] No node is currently in CS!")

        elif choice == "3":
            print("\n--- SYSTEM STATUS ---")
            for node in nodes:
                print(f"Node {node.id} | Clock: {node.clock} | "
                      f"Requesting: {node.requesting} | In CS: {node.in_cs}")
            print("---------------------")

        elif choice == "4":
            print("Exiting simulation...")
            break

        else:
            print("Invalid choice!")

simulate()
